...
elev_ts=
	plot_ly() %>%
		add_lines(data=subset(elevation, Bay=="Gilbert"), y=~elev_ft, x=~Date, name="Gilbert Bay (South arm)",
			line = list(color = cols[[5]][[2]],width = 2)) %>%
		add_lines(data=subset(elevation, Bay=="Gunnison"), y=~elev_ft, x=~Date, name="Gunnison Bay (North arm)",
			line = list(color = cols[[4]][[2]],width = 2)) %>%
		layout(title = "Lake elevation",
			xaxis = list(title = "", range=c(as.numeric(gauge_date_range[2]-365*50)*86400000,as.numeric(gauge_date_range[2])*86400000), rangeslider = list(type = "date")),
