...
current_na_sal=mean(subset(wq_data, site_no==10010100 & sample_dt>=gauge_date_range[1]-30 & sample_dt<=gauge_date_range[2])$sal_pct, na.rm=T)
...
sal_ts=
	plot_ly(x=~sample_dt) %>%
		...
		#) %>% 
		add_markers(data=wq_data[wq_data$rel_depth=="Surface" & wq_data$Bay=="Gilbert" & wq_data$SiteName!="GSL FARMINGTON BAY OUTFLOW AT CAUSEWAY BRIDGE" & wq_data$site_no!=411116112244401,], x=~sample_dt, y=~sal_pct, name="Gilbert surface", visible=T, 
			text=~site_no,
